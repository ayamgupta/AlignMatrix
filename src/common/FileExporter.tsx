 ...
 * @param props 
 */
export async function downloadFullViewportSVG(props: {
  alignment: Alignment, 
  svgFilename?: string;
  ...
  // For worker-backed large alignments, sequences aren't in memory.
  // Pre-fetch the exact slice the export needs before generating the SVG.
  let prefetchedSequences: ISequence[] | undefined;
  if (alignment.getSlice) {
    const startIdx = props.startSeqIdx ?? 0;
    const endIdx = props.endSeqIdx !== undefined
      ? props.endSeqIdx
      : alignment.getSequenceCount() > 100 ? 99 : alignment.getSequenceCount() - 1;
    const { sequences: seqStrs, annotations: anns } = await alignment.getSlice(startIdx, endIdx + 1);
    prefetchedSequences = seqStrs.map((sequence, i) => ({
      sequence,
      annotations: anns[i] ?? {},
    }));
  }

  const alignmentString = getFullViewportSvgString({ ...props, prefetchedSequences });
...
const getFullViewportSvgString = (props: {
  ...
  /** Pre-fetched sequences for worker-backed alignments (avoids getSequences()). */
  prefetchedSequences?: ISequence[];
  alignmentType: AminoAcidAlignmentTypeInstance | NucleotideAlignmentTypeInstance;
  positionsToStyle: PositionsToStyleInstance;
  ...
  // Use pre-fetched sequences when available (large worker-backed files),
  // otherwise fall back to the in-memory getSequences() call.
  const prefetchedSequences = props.prefetchedSequences;
  const seqs = prefetchedSequences ?? alignment.getSequences(sortBy);
  const exportedSeqs = prefetchedSequences
    ? seqs  // already the exact slice we need (indexed from 0)
    : startEndIdxToArray(startSeqIdx, endSeqIdx).map(seqIdx => seqs[seqIdx]);
