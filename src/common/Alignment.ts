...
export class Alignment {
   ...
   * sequences are NOT stored in sortedSequencesCache. Instead this holds the
   * count, and getSlice() must be used to fetch rows on demand.
   */
  private workerSequenceCount: number | undefined = undefined;

  /**
   * Callback to fetch a slice of sequences from the worker.
   * Only set when fromWorkerMetadata() was used.
   */
  public getSlice:
    | ((start: number, end: number, sortKey?: string) => Promise<{ sequences: string[]; annotations: any[] }>)
    | undefined = undefined;
  ...
  public constructor(props: {
    ...
    //A3M files have lowercase insertion letters - strip them first if needed.
    ...
    if (Object.keys(sequenceLengths).length > 1) {
      // A3M files have unequal lengths due to insertion columns (lowercase).
      // Pad all sequences with trailing "-" to the longest length.
      const maxLen = Math.max(...Object.keys(sequenceLengths).map(Number));
      for (let i = 0; i < finalInputSequences.length; i++) {
        const seq = finalInputSequences[i].sequence;
        if (seq.length < maxLen) {
          finalInputSequences[i] = {
            ...finalInputSequences[i],
            sequence: seq + "-".repeat(maxLen - seq.length),
          };
        }
      }
      // Recompute lengths after padding
      sequenceLengths = {};
      for (const s of finalInputSequences) sequenceLengths[s.sequence.length] = true;
      if (Object.keys(sequenceLengths).length > 1) {
        throw Error(
  ...
  getSequenceCount(): number {
    if (this.workerSequenceCount !== undefined) return this.workerSequenceCount;
  ...
  static getPositionalLetterColors = (props: {
    ...
    // Use the longest non-empty sequence to determine column count.
    // sequences[0] may be "" (async-guard placeholder) while other rows
    // already have content, which would otherwise leave msaColors empty
    // and cause a crash when MSALetters iterates non-empty sequences.
    const seqLenFromAll = sequences.reduce((max, s) => Math.max(max, s.length), 0);

    for(
      var posIdx = 0, seqLen = seqLenFromAll;
      posIdx < seqLen;
      posIdx++
    ){
  ...
  // -------------------------------------------------------------------------
  // Pre-computed constructor (used when deserializing from a Web Worker)
  // -------------------------------------------------------------------------

  /**
   * Exported shape of pre-computed alignment data that can be safely
   * postMessage'd across Worker boundaries (plain transferable objects).
   */
  // NOTE: this interface is also used in AlignmentParserWorker.ts
  // (duplicated there to avoid importing from the main bundle).

  /**
   * Reconstruct an Alignment from pre-computed data that was calculated
   * in a Web Worker.  All heavy computation is skipped; fields are simply
   * assigned from the provided values.
   */
  static fromPrecomputed(data: {
    name: string;
    uuid: string;
    sequences: ISequence[];
    maxSequenceLength: number;
    ...
    querySequence: ISequence;
    consensus: ISequence;
    allRepresentedCharacters: string[];
    allUpperAlphaLettersInAlignmentSorted: string[];
    // positionalLetterCounts serialised as [posIdx, {letter: count}][]
    positionalLetterCounts: [number, { [letter: string]: number }][];
    globalAlphaLetterCounts: { [letter: string]: number };
    annotationFields: Record<string, { key: string; name: string }>;
  }): Alignment {
    // Use Object.create to bypass the constructor entirely
    const inst = Object.create(Alignment.prototype) as Alignment;
    (inst as any).uuid = data.uuid;
    (inst as any).name = data.name;
    (inst as any).maxSequenceLength = data.maxSequenceLength;
    (inst as any).predictedNT = data.predictedNT;
    (inst as any).numberDuplicateSequencesInAlignment = data.numberDuplicateSequencesInAlignment;
    (inst as any).numberRemovedDuplicateSequences = data.numberRemovedDuplicateSequences;
    (inst as any).querySequence = data.querySequence;
    (inst as any).consensus = data.consensus;
    (inst as any).allRepresentedCharacters = data.allRepresentedCharacters;
    (inst as any).allUpperAlphaLettersInAlignmentSorted = data.allUpperAlphaLettersInAlignmentSorted;
    (inst as any).positionalLetterCounts = new Map(data.positionalLetterCounts);
    (inst as any).globalAlphaLetterCounts = data.globalAlphaLetterCounts;
    (inst as any).annotationFields = data.annotationFields;
    (inst as any).sortedSequencesCache = new Map([[SequenceSorter.INPUT, data.sequences]]);
    return inst;
  }


  /**
   * Reconstruct an Alignment from worker metadata WITHOUT sequences.
   * Sequences remain in the Worker and are fetched on demand via getSlice().
   *
   * @param data  The IWorkerMetadata object from AlignmentParserWorker
   * @param getSliceFn  Function that asks the worker for a row range
   */
  static fromWorkerMetadata(
    data: {
      name: string;
      uuid: string;
      sequenceCount: number;
    ...
    getSliceFn: (start: number, end: number) => Promise<{ sequences: string[]; annotations: any[] }>
  ): Alignment {
    const inst = Object.create(Alignment.prototype) as Alignment;
    (inst as any).uuid = data.uuid;
    (inst as any).name = data.name;
    ...
    (inst as any).globalAlphaLetterCounts = data.globalAlphaLetterCounts;
    (inst as any).annotationFields = data.annotationFields;
    (inst as any).workerSequenceCount = data.sequenceCount;
    // Empty cache — sequences are fetched on demand via getSlice
    (inst as any).sortedSequencesCache = new Map([[SequenceSorter.INPUT, []]]);
    inst.getSlice = getSliceFn;
    return inst;
  }
