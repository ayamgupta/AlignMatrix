 ...
 *     fromLineIterator: accepts an async line iterator for streaming large files
 */
export class StockholmAlignment extends Alignment {
      ...
      .forEach((line, idx) => {
        ...
        if (line && !line.startsWith("#")) {
          ...
          if (
            ...
            StockholmAlignment._parseMetadataLine(line, metadata);
          }
        }
      });

    StockholmAlignment._finalizeSequences(sequences, metadata);
  ...
  /**
   * Parse a Stockholm alignment from an async line iterator (streaming, low-memory).
  ...
  static async fromLineIterator(
    ...
    try {
      for await (const rawLine of lineIterator) {
        ...
        if (lineIndex === 0) {
          if (!line.startsWith("# STOCKHOLM 1.0")) {
            throw getParseError("Stockholm", "The first line must be '# STOCKHOLM 1.0'");
          }
          lineIndex++;
          continue;
        }
        ...
        if (line && !line.startsWith("#")) {
          ...
          if (
            ...
            console.warn("Unexpected metadata line in Stockholm alignment: " + line);
          } else {
            StockholmAlignment._parseMetadataLine(line, metadata);
          }
        }
      ...
      if ((e as Error).name === "Stockholm Parse Error") throw e;
      throw getParseError("Stockholm", (e as Error).message);
    }
  ...
  /**
   * Parse a single metadata line and update the metadata object in-place.
   * Shared between the string-based and streaming parsers.
   */
  private static _parseMetadataLine(
    line: string,
    metadata: IStockholmAlignmentMetadata
  ): void {
  ...
  /**
   * Post-process sequences to attach full annotations from metadata.
   * Shared between the string-based and streaming parsers.
   */
  private static _finalizeSequences(
    sequences: ISequence[],
    metadata: IStockholmAlignmentMetadata
  ): void {
