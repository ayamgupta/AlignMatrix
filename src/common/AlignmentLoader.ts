...
export class AlignmentLoader {
   ...
   */
  public static onProgress: ((message: string) => void) | undefined = undefined;
  ...
  public static loadAlignmentFromText(
    ...
    for (const AlignmentType of AlignmentLoader.AlignmentFileTypes) {
      try {
        return AlignmentType.fromFileContents(alignmentName, text, removeDuplicateSequences);
      } catch (e) {
  ...
  private static _loadViaWorker(
    ...
    const pendingSlices = new Map<number, {
      resolve: (v: { sequences: string[]; annotations: any[] }) => void;
      reject: (e: Error) => void;
    }>();
    ...
    worker.onmessage = (event) => {
      ...
      if (msg.type === "slice") {
        ...
        if (pending) {
          ...
          pending.resolve({ sequences: msg.sequences, annotations: msg.annotations });
        }
        return;
      }
      ...
      if (msg.type === "done") {
        try {
          ...
          worker.terminate();
          errorCallback(
            new AlignmentLoadError("Failed to reconstruct alignment", [
        ...
        errorCallback(
          new AlignmentLoadError(
            msg.message,
            msg.errors ?? [{ name: msg.name, message: msg.message }],
    ...
    worker.onerror = (e) => {
      worker.terminate();
      errorCallback(
        new AlignmentLoadError("Worker error during alignment parsing", [
