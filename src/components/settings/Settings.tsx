...
import {
    SequenceSorter,
    SequenceSorterInstance,
} from "../../common/AlignmentSorter";
import {
...
export default function useAV2Settings(props: {
    ...
    const { requestSettingsClose, useUrlAndLocalstorage = false } = props;
    ...
    const properties = useMemo(() => {
        function populateObj<T>(props: {
            ...
            let initCache =
                globalSettingsUrlLocalStorageManager.getCurrentDeserializedValue(
                    propKey,
                    deserialize,
                );
            if (
                ...
                initCache === undefined ||
                !all.includes(initCache)
            ) {
                //set default if not found or not valid
        ...
        return {
            alignmentType: populateObj({
                propKey: "alignment-type",
                stateKey: "alignmentType",
            ...
            ntColorScheme: populateObj({
                propKey: "nt-color-scheme",
                stateKey: "ntColorScheme",
            ...
            aaColorScheme: populateObj({
                propKey: "aa-color-scheme",
                stateKey: "aaColorScheme",
            ...
            positionsToStyle: populateObj({
                propKey: "positions-to-style",
                stateKey: "positionsToStyle",
            ...
            residueColoring: populateObj({
                propKey: "residue-coloring",
                stateKey: "residueColoring",
            ...
            showMinimap: populateObj({
                propKey: "show-minimap",
                stateKey: "showMinimap",
                defaultVal: defaultGlobalSettings.showMinimap,
                ...booleanHelper,
            }),
            showAnnotations: populateObj({
                propKey: "show-annotations",
                stateKey: "showAnnotations",
            ...
            removeDuplicateSequences: populateObj({
                propKey: "remove-duplicates",
                stateKey: "removeDuplicateSequences",
    ...
    }, [defaultGlobalSettings, useUrlAndLocalstorage]);
    ...
    const barplots = useMemo(() => {
        ...
        const serialize = (vals: IPositionalBarplotDataSeries[]) => {
            let toreturn = vals
                .map((v) => PreconfiguredPositionalBarplots.serialize(v)!)
                .toSorted()
                .join("|");
        ...
        if (
            useUrlAndLocalstorage &&
            initCacheVals !== undefined &&
            initCacheVals.length > 0
        ) {
    ...
    }, [defaultGlobalSettings.barplots, useUrlAndLocalstorage]);

    useEffect(() => {
        const validPropKeys = Object.keys(properties).reduce(
            (acc, key) => {
                acc.push(properties[key as keyof typeof properties].propKey);
                return acc;
            },
            [barplots.propKey] as string[],
        );

        globalSettingsUrlLocalStorageManager.setValidSavePropKeys(
            validPropKeys,
        );
    }, [barplots, properties]);
    ...
    const [state, setState] = useState<ISettingsState>({
        ...
        removeDuplicateSequences:
            properties.removeDuplicateSequences.initialValue,
    ...
    const alignmentTypeChanged = useCallback(
        (
            ...
            if (
                (newAlignmentType === AlignmentTypes.AMINOACID &&
                    (!state.sortBy ||
                        !SequenceSorter.ALL_AMINO_ACID_SORTERS.includes(
                            state.sortBy,
                        ))) ||
                (newAlignmentType === AlignmentTypes.NUCLEOTIDE &&
                    (!state.sortBy ||
                        !SequenceSorter.ALL_NUCLEOTIDE_SORTERS.includes(
                            state.sortBy,
                        )))
            ) {
                setState((prevState) => {
        ...
        [properties.sortBy.defaultValue, state.sortBy],
    );
    ...
    const startAlignmentLoading = useCallback(() => {
        AlignmentLoader.onProgress = (message: string) => {
            setState((prevState) => ({ ...prevState, loadingStatus: message }));
        };
        // When background stats finish, force a re-render so logo/barplots update
            ...
            setState((prevState) => {
                if (prevState.alignment === updatedAlignment) {
                    ...
                    return {
                        ...
                        statsVersion: prevState.statsVersion + 1,
                    };
                }
                return prevState;
            });
    ...
    //
    const onAlignmentReceived = useCallback(
        (alignment: Alignment) => {
            setState((prevState) => {
                return {
                    ...prevState,
    ...
    useEffect(() => {
        if (!onceHappened.current) {
            ...
            const potentialUrl =
                globalSettingsUrlLocalStorageManager.getAlignmentUrlParam();
            const potentialName =
                globalSettingsUrlLocalStorageManager.getAlignmentNameParam();
            ...
            if (potentialUrl) {
                try {
                    ...
                    onAlignmentLoadError(
                        new AlignmentLoadError("Alignment URL Parse Error", [
                            {
                                name: "URL invalid",
    ...
    const renderedSettings = useMemo(() => {
        return (
            <div className={"settings-content"}>
                <h3 className="subheader">Alignment Styling</h3>
                ...
                <h3 className="subheader">Analysis</h3>
                <div className="options-box">
                    <label htmlFor={barplots.propKey}>Barplots:</label>
                    <InputMultiSelectSetting
                        {...barplots}
                    ...
                    <InputSelectSetting
                        ...
                        onChange={(newLogoType) => {
                            if (!newLogoType) {
                                //hide-logo is selected - undefined in LogoType.deserialize
                                setState((prevState) => {
                                    return {
                                        ...prevState,
                                        showLogo: false,
          ...
          */}
                ...
                <div className="options-box">
                    <label
                        htmlFor={properties.removeDuplicateSequences.propKey}
                    >
