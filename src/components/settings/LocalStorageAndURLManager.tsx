...
export const globalSettingsUrlLocalStorageManager = (() => {
    ...
    return {
        ...
        getAlignmentUrlParam: () => {
            ...
            const resultsPath = params.get("resultsPath");
            const isRootWithoutParams =
                window.location.pathname === "/" &&
                window.location.search === "";
            if (resultsPath || isRootWithoutParams) {
                return `http://localhost:8000/alignment-file`;
            }
            const url = params.get(ALIGNMENT_URL_KEY);
        ...
        getAlignmentNameParam: () => {
            //special case
            const params = getCurrentParams();
            const name = params.get("alignment-name");
            if (name) return name;
