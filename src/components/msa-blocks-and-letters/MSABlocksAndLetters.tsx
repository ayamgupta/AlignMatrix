...
export type MSABlocksAndLettersProps = {
   ...
   * When provided, overrides the internal alignment.getSequences() call so
   * the component never tries to load all sequences at once.
   */
...
export function MSABlocksAndLetters(props: MSABlocksAndLettersProps) {
  ...
  const sliceSequences = useCallback((
    ...
    for(let seqIdx = firstRowIdx; seqIdx <= lastRowIdx; seqIdx++){
      ...
      // Guard: sequence may be undefined while async slice is in flight
      if (!seq) { toReturn.push(""); continue; }
      const slicedSeq = [];
  ...
  /**
   * Like sliceSequences but accepts absolute row indices and converts them
   * to relative indices using the provided offset. Used for external slices.
   */
  const sliceExternalSequences = useCallback((
    sequencesToSlice: string[],
    offset: number,
    firstRowIdx: number, lastRowIdx: number,
    firstColIdx: number, lastColIdx: number
    ...
    const toReturn: string[] = [];
    for(let seqIdx = firstRowIdx; seqIdx <= lastRowIdx; seqIdx++){
      const relIdx = seqIdx - offset;
      const seq = relIdx >= 0 ? sequencesToSlice[relIdx] : undefined;
      if (!seq) { toReturn.push(""); continue; }
      const slicedSeq = [];
      for(let posIdx = firstColIdx; posIdx <= lastColIdx; posIdx++){
        slicedSeq.push(seq[posIdx] ?? "-");
  ...
  const sequences = useMemo(()=>{
    ...
    //
    // For large worker-backed alignments, externalSequences is a pre-fetched
    // slice of the visible rows. Use it directly to avoid loading all rows.
    if (sequenceSet === "alignment" && externalSequences !== undefined) {
  ...
  const renderMatrixContent = useCallback(({
    ...
    // When externalSequences is in use, the virtualizer's row indices are absolute
    // (e.g. row 500 of 1,000,000) but sequences[] is a relative slice starting at
    // externalSequencesOffset. Convert before indexing.
    const rowBase = externalSequences !== undefined ? externalSequencesOffset : 0;
    const seqsSliced = sliceSequences(
      sequences, 
      firstRowIdxToRender - rowBase, lastRowIdxToRender - rowBase, 
      firstColIdxToRender, lastColIdxToRender
    );
    ...
    const fullSequencesInViewport = startEndIdxToArray(
      ...
      return sequences[seqIdx - rowBase] ?? "";
    }, []);
