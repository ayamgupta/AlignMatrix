...
export interface MSABlocksProps {
  ...
  /**
   * For streaming (large-file) mode: the absolute row index of sequences[0]
   * in the full alignment. The WebGL stage position must be offset by this
   * amount so the canvas window is placed at the correct vertical position
   * rather than always starting at row 0.
...
export const MSABlocks = (props: MSABlocksProps) => {
  ...
  //IMPORTANT: do NOT include the raw `sequences` array reference — in streaming
  //mode it changes on every scroll (new window of rows), which would trigger a
  //full canvas regeneration + setLoading flicker on every scroll event.
  //Instead we use a stable fingerprint: row-count, row-offset, and seq-length.

  // Stable sequence-length primitive. getPositionalLetterColors only uses the
  // sequences array to determine column count (not per-row content), so we
  // stabilise on a number to avoid recomputing on every streaming scroll that
  // delivers a new externalSequences array reference with the same length.
  const sequenceLength = sequences.length > 0 ? sequences[0].length : 0;
  ...
  // web workers
  //
  // Track whether the last submitted job was a style change (colorScheme,
  // residueColoring, etc.) vs. just a sequence-window scroll. We only blank
  // the canvas and show the loading indicator for true style changes, not for
  // the normal streaming scroll that just shifts the window of visible rows.
  const prevStyleKeyRef = React.useRef<string>("");
  const styleOnlyKey = useMemo(()=>{
    // Everything in canvasUniqueKey EXCEPT sequences.length / sequencesRowOffset
    return [
      querySequence,
      consensusSequence,
      alignmentType.key,
  ...
  useEffect(()=>{
    ...
    // Only clear the canvas and show loading for true style/coloring changes,
    // NOT for streaming scroll (which just changes which window of rows is loaded).
    // Without this guard, every scroll in streaming mode causes a blank flash.
    const isStyleChange = styleOnlyKey !== prevStyleKeyRef.current;
    prevStyleKeyRef.current = styleOnlyKey;
  ...
  useEffect(()=>{
    if(
      ...
      // In streaming mode the canvas only contains rows starting at sequencesRowOffset,
      // not from row 0. We need to shift the stage UP by the offset amount so that
      // row `sequencesRowOffset` in the canvas aligns with its correct world position.
      // Without this correction, the colors would only appear correct for the first
      // window of rows (e.g. the initial 121 rows), and everything else would be blank.
      const correctedPositionY = positionY + sequencesRowOffset * scaleY;
      webglApp.stage.position.set(positionX, correctedPositionY);
      webglApp.render();
      ...
      if(context){
        ...
        context.drawImage(
          ...
          //For streaming mode: correct for sequencesRowOffset so the canvas window
          //is drawn at the right vertical position in the viewport.
          -positionX/scaleX, (-positionY + sequencesRowOffset * scaleY)/scaleY, bb.width/scaleX, bb.height/scaleY,
