...
export function MiniMap(props: IMiniMapProps) {
  ...
  // For large files (worker-backed), the minimap receives the already-loaded
  // visible slice rather than all sequences. This avoids OOM on huge alignments.
  // The minimap will show a partial view but won't crash the page.
  const sequences = useMemo(()=>{
    //NOTE - any additional changes to sequences (like sorting) must be propogated as
    //keys for MSAblocks
    if (alignment.getSlice) {
      // Large file: use whatever slice AlignmentViewerHook has already loaded
      return externalSequences ?? [];
    }
    return alignment.getSequences(sortBy).map(s=>s.sequence);
  }, [alignment, sortBy, externalSequences]);
