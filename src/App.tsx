...
export default function App() {
    ...
    const [state, setState] = useState(() => {
        ...
        return {
            ...
            mainViewportVisibleIdxs: undefined as
                | undefined
                | {
                      seqIdxStart: number;
                      seqIdxEnd: number;
    ...
    useMemo(() => {
        ...
        if (resultsPath || isRootWithoutParams) {
            const fetchUrl = `http://localhost:8000/alignment-file`;
            let updated = false;
    ...
    useEffect(() => {
        ...
        return () => {
            AlignmentLoader.onSortUpdate = undefined;
        };
    ...
    }, [alignment, barplots, statsVersion]);
    ...
    const renderedSettingsBox = useMemo(() => {
        const alignmentDescription = alignment ? (
            <>
                <h3>
                    <strong>Alignment:</strong> {alignment.getName()}
                </h3>
                <h4>
                        ...
                        <form>
                            <div className="settings-header">
                                        ...
                                        if (alignment) {
                                            downloadFullViewportSVG({
                                                ...
                                                residueColoring:
                                                    residueColoring,
                                                logoSvgId: logoSvgId,
                                                barplots: barplotsProps.map(
                                                    (bp) => {
                                                        return {
                                                            svgId: bp.svgId,
                                                            title: bp.dataSeriesSet
                                                                .map(
                                                                    (ds) =>
                                                                        ds.description,
                                                                )
                                ...
                                <button
                                    ...
                                    title="Show Search"
                                    onClick={() => {
                                        if (triggerShowSearch.current)
                                            triggerShowSearch.current();
                                    }}
                                >
                                    <img
                                        alt="Show Search"
    ...
    return (
        <>
            {!alignmentLoading &&
                (alignment || showSettings) &&
                renderedSettingsBox}
            <div
                className={`fullscreen-loading-indicator ${alignmentLoading || settings.currentlySelectedProperties.alignmentLoadError ? "" : "hidden"}`}
            >
                {settings.currentlySelectedProperties.alignmentLoadError ? (
                    <div className="error-container">
                        ...
                        <div className="loading-status-text error-message">
                            {
                                settings.currentlySelectedProperties
                                    .alignmentLoadError.message
                            }
                        </div>
                        {settings.currentlySelectedProperties.alignmentLoadError.errors.map(
                            (err, i) => (
                                <div
                                    key={i}
                                    className="loading-status-text error-detail"
                                >
                                    {err.message}
                                </div>
                            ),
